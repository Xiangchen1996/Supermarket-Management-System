...
bool JsonParser::getReturn()
{
    ...
    if (error.error == QJsonParseError::NoError)
    {
           if (doucment.isObject())
           {
               ...
               {
                   QJsonValue ret_value = obj.take("return");
                   if (ret_value.isBool())
                   {
                       ret = ret_value.toVariant().toBool();
                   }
               }
           }
    }
...
QString JsonParser::getSearchType()
{
    QJsonParseError error;
    QString searchType;
    QJsonDocument doucment = QJsonDocument::fromJson(message.toUtf8(),&error);
    if (error.error == QJsonParseError::NoError)
    {
           ...
           {
               QJsonObject obj = doucment.object();
               if (obj.contains("searchType"))
               {
                   QJsonValue searchType_value = obj.take("searchType");
                   if (searchType_value.isString())
                   {
                       searchType = searchType_value.toVariant().toString();
                   }
               }
           }
    }
...
QJsonArray JsonParser::getOutStockList()
{
    //code:qty
    QJsonParseError error;
    QJsonDocument doucment = QJsonDocument::fromJson(message.toUtf8(),&error);
    ...
    if (error.error == QJsonParseError::NoError)
    {
           if (doucment.isObject())
           {
                 ...
                 {
                       QJsonValue arrays_value = jsonObject.take("items");
                       if (arrays_value.isArray())
                       {
                                  itemsArray = arrays_value.toArray();;
                       }
                 }
           }
    }
    qDebug()<<"itemsArray:"<<itemsArray;
    return itemsArray;
}
